...
Teapot::Teapot(String name): AGameObject(name)
{
	...
	this->setObjectTexture(TextureLibrary::getInstance()->getTexture(filenames.BRICK));
	this->setTextureFlag();
...
void Teapot::draw(int width, int height)
{
	...
	device->setVertexBufferTextured(this->teapot->getVertexBuffer());
	device->setIndexBuffer(this->teapot->getIndexBuffer());

	device->drawIndexedTriangleList(this->teapot->getIndexBuffer()->getSizeIndexList(), 0, 0);
